        ...
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                ...
                if os.path.isdir(arg.model_saved_name):
                    ...
                    print('Refresh the website of tensorboard by pressing any keys')
                    # else:
    ...
    def train(self, epoch, save_model=True):
        ...
        for batch_idx, (joint, data, label, index) in enumerate(process):
            ...
            with torch.no_grad():
                ...
                joint = data.cuda(self.output_device)
                index = label.cuda(self.output_device)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (joint, data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    data = data.float().cuda(self.output_device)
                    label = label.long().cuda(self.output_device)
                    joint = joint.cuda(self.output_device)
                    # for mmd
                    if arg.mmd:
            ...
            if save_score:
                np.save("{}/epoch{}_{}_pred.npy".format(self.arg.work_dir, epoch + 1, ln), score)
