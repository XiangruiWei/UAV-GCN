...
class Feeder(Dataset):
    def __init__(self, data_path, label_path, p_interval=1,
                 ...
                 bone=False, vel=False, frame=False):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.vel:
            mask = np.all(np.diff(data_numpy, axis=1) != 0, axis=(1, 2, 3))
            data_diff = np.diff(data_numpy, axis=1)
            data_numpy[mask, :-1] = data_diff[mask]
            data_numpy[mask, :-1] = 0
        if self.frame and self.vel:
            data_numpy /= 0.04
